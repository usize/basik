...
function evaluateExpression(expr, scopes) {
  ...
  if (expr.type == "UnaryOp") {
    return unaryOps[expr.op.value](evaluateExpression(expr.exprR, scopes));
  }
  if (expr.type == "BinOp") {
    if (expr.op.value == "ASSIGN")
      return defineVariable(
        expr.exprL.variable.value,
        evaluateExpression(expr.exprR, scopes),
        scopes
      );
    return binaryOps[expr.op.value](evaluateExpression(expr.exprL, scopes),
                              evaluateExpression(expr.exprR, scopes));
  }
  if (expr.type == "Call") {
