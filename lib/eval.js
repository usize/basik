  ...
  var step = (expNumber) => {
    ...
    for (var [i, command] of stmt.commands.entries()) {
      ...
      if (command.type == "Goto") {
        ...
        scopes.__functionData[command.name] =  command;
        ...
        while (command.args.length) {
            for (var i=0; i < arrays.length; i++)
              arrays[i] = new Array(evaluateExpression(command.args[command.args.length - 1], scopes));
            command.args.pop();
        }
        scopes.__functionData[command.name] = arrays;
      } else if (command.type == "DimAssign") {
        var dim = scopes.__functionData[command.name];
        dim[evaluateExpression(command.innerExpr, scopes)] = evaluateExpression(command.exprR, scopes);
        ...
        if (!command.initialized) {
          ...
          if (command.stepExpr !== undefined)
            command.stepValue = evaluateExpression(command.stepExpr, scopes)
          command.decrementing = (command.stepValue * -1) > 0 ? true : false;
          currentValue = lookupVariable(command.nextVar.fullName, scopes);
          command.initialized = true;
        } else {
          currentValue = lookupVariable(command.nextVar.fullName, scopes);
          defineVariable(command.nextVar.fullName, currentValue + command.stepValue, scopes);
        }

        toValue = evaluateExpression(command.toExpr, scopes)
        if ((command.decrementing && (currentValue > toValue)) ||
            !command.decrementing && (currentValue < toValue)) {
          returnStack.push(command.trueTarget - 1);
          pc = command.trueTarget;

          // We need to evaluate the step expressoin during each iteration
          if (command.stepExpr !== undefined)
            command.stepValue = evaluateExpression(command.stepExpr, scopes)
...
function evaluateExpression(expr, scopes) {
  ...
  if (expr.type == "Call") {
    ...
    var userFunc = scopes.__functionData[fnname];
