...
function computeVarBindings(program) {
  ...
  for (var stmt of program) {
    ...
    if ("expr" in cmd) {
      var numSubExprs = collectStrings(strings, stringsArray, cmd.expr);
      numExprs = Math.max(numExprs, numSubExprs);
    }
    if (stmt.command.type != "Let" && stmt.command.type != "Input")
      continue;
  ...
  if (typeof(expr) == "string") {
    ...
    switch (expr.type) {
      ...
      case "BinOp":
        ...
        exprCode += stmtindent + "e" + lastVar +" = ((e"+ lhs.lastVar + op + "e" + rhs.lastVar + ")<<16)>>16;";
        break;
    }
  }
